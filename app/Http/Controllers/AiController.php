...
class AiController extends Controller
{
    public function generateAiPage(Request $request) {
        ...
        if(empty($queryTitle)){
        return view('ai.generate-feed-content-with-ai');
        }
        ...
        if ($response->successful()) {
            ...
            return view('ai.generate-feed-content-with-ai', ['data' => $data]);
        } else {
            $error = $response->body();
            return view('ai.generate-feed-content-with-ai', ['error' => $error]);
        }
    }
}
